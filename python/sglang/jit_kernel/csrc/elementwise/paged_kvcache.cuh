...
struct StorePagedKVCacheParams {
  const void* __restrict__ k;
  const void* __restrict__ v;
  void* __restrict__ kv_storage;
  const void* __restrict__ indices;  // loc: [B], token slot ids
  int64_t stride_k_bytes;
  int64_t stride_v_bytes;
  int64_t stride_cache_bytes;    // = row_bytes (shared by K and V slices)
  int64_t page_stride_bytes;     // = 2 * num_layers * page_size * row_bytes
  int64_t layer_offset_bytes;    // = layer_idx * page_size * row_bytes
  int64_t kv_half_stride_bytes;  // = num_layers * page_size * row_bytes (K->V byte offset)
...
/**
 * \brief Kernel to store key-value pairs into a paged [P, 2, L, S, H, D] cache.
 * Destination row indices are derived in-kernel from the per-token ``loc`` id
 * plus ``layer_idx`` / ``page_size``, so no Python-side index tensor is needed.
 * \tparam kElementBytes The size of each key/value element in bytes.
...
__global__ void store_paged_kvcache(const __grid_constant__ StorePagedKVCacheParams params) {
  using namespace device;
  ...
  const auto& [k_input, v_input, kv_storage_ptr, indices, stride_k, stride_v, stride_cache, page_stride, layer_offset, kv_half, stride_indices, batch_size, page_size] =
      params;
  ...
  const auto loc = *index_ptr;
  // Derive paged destination row from loc without any host-side index tensor.
  const T page_size_t = static_cast<T>(page_size);
  const T page_idx = loc / page_size_t;
  const T slot = loc - page_idx * page_size_t;
  const int64_t k_row_bytes =
      static_cast<int64_t>(page_idx) * page_stride + layer_offset + static_cast<int64_t>(slot) * stride_cache;
  ...
  const auto k_dst = pointer::offset(kv_storage_ptr, k_row_bytes, split_id * kSplitSize);
  // V shares the same storage at offset kv_half bytes from the K row.
  const auto v_dst = pointer::offset(k_dst, kv_half);
...
struct StorePagedKVCacheKernel {
  static_assert(kElementBytes > 0 && kElementBytes % 4 == 0);

  template <int kSplit, typename T>
  static constexpr auto store_kernel = store_paged_kvcache<kElementBytes, kSplit, kUsePDL, T>;
  ...
  run(const tvm::ffi::TensorView k,
      ...
      const tvm::ffi::TensorView kv_storage,  // [rows, D] 2-D view of paged buffer
      const tvm::ffi::TensorView indices,     // [B]
      int64_t layer_idx,
      int64_t page_size,
      int64_t num_layers,
      const int num_split) {
    ...
    RuntimeCheck(page_size > 0 && num_layers > 0 && layer_idx >= 0 && layer_idx < num_layers);

    const int64_t row_bytes = S.unwrap() * dtype_size;
    const int64_t kv_half_rows = num_layers * page_size;
    const int64_t page_stride_rows = 2 * kv_half_rows;
    const int64_t layer_offset_rows = layer_idx * page_size;

    const auto params = StorePagedKVCacheParams{
        .k = k.data_ptr(),
        .v = v.data_ptr(),
        .kv_storage = kv_storage.data_ptr(),
        .indices = indices.data_ptr(),
        .stride_k_bytes = KS.unwrap() * dtype_size,
        .stride_v_bytes = VS.unwrap() * dtype_size,
        .stride_cache_bytes = row_bytes,
        .page_stride_bytes = page_stride_rows * row_bytes,
        .layer_offset_bytes = layer_offset_rows * row_bytes,
        .kv_half_stride_bytes = kv_half_rows * row_bytes,
